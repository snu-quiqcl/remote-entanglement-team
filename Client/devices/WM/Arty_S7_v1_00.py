...
"""
dac.send_mod_BTF_int_list([0xff,0x04, 0x00, 0x05])
dac.send_command('WRITE REG')
dac.send_mod_BTF_int_list([0xff,0x00, 0x40, 0x3C])    
dac.send_command('WRITE REG')

dac.send_mod_BTF_int_list([0xff,0x00, 0x00, 0x3C])  
dac.send_mod_BTF_int_list([0xff,0x00, 0x03, 0x3C])  
dac.send_command('WRITE REG')

for ch in range(4):
    dac.send_mod_BTF_int_list([0xff,0x04+ch, 0x80, 0x05])
    dac.send_command('WRITE REG')

dac.send_mod_BTF_int_list([0xff,0x00, 0x43, 0x3C])    
dac.send_command('WRITE REG')
...
dac.update_bit_pattern([(1,1), (3,0), (4,0), (5,1), (7, 1), (10, 1)])
dac.update_bit_pattern([(1,0), (3,1)])
dac.update_bit_pattern([(32,1)])
        
dac.read_bit_pattern()
...
def send_dummy_BTF(length, start):
    ...
    #print(data_list)
    dac.send_mod_BTF_int_list(data_list)
...
dac.send_command('CAPTURE BTF')

# 'BTF READ COUNT' command sets how many bytes 'READ BTF' command will read
# dac.send_mod_BTF_string('\x01\x00') means read all 256 bytes in BTF_capture
dac.send_mod_BTF_string('\x01\x00')
dac.send_command('BTF READ COUNT')

# 'READ BTF' will read BTF_capture[1:('BTF READ COUNT')*8]
dac.send_command('READ BTF')
dac.read_next_message()

# The following are testing non-escape character situation
dac.com.write(b'!6\x10\x10TEST\x10\x10\r\n') # dac.send_command('\x10TEST\x10')
dac.read_next_message()
...
dac.com.write(b'!6\x10\x10TEST\x10\r\n') # Wrong format
dac.read_next_message()
...
after_trigger_count % 256
dac.send_mod_BTF_int_list([int('11000000', 2), int('00111111', 2), 
                           int('10101001', 2), int('10010000', 2), 
                           after_trigger_count // 256, after_trigger_count % 256])
dac.com.write(b'\x10T')


dac.check_waveform_capture() # Check the status of trigger
dac.send_command('WAVEFORM') # Trigger from main FSM. This is sample implementation
dac.check_waveform_capture() # Check the status of trigger
dac.com.write(b'\x10W') # If any captured waveform data exists, read those waveform data
dac.read_next_message() # This will raise b'\x10W' detection
dac.com.inWaiting() # returns the bytes waiting in the first-level buffer (max ~4,000)
dac.read_captured_waveform(True) # Read one block of actual waveform
dac.com.inWaiting()
...
dac.check_waveform_capture() # Check the status of trigger
dac.com.write(b'\x10A') # similar to dac.send_command('WAVEFORM'), but the trigger is armed through escape character, therefore it will work even if the main FSM is running in infinite loop
dac.check_waveform_capture() # Check the status of trigger
dac.com.write(b'\x10W') # Read waveform data
dac.read_next_message() # This will raise b'\x10W' detection
dac.read_captured_waveform(True) # Read one block of actual waveform
...
def test_trigger_pattern_and_delay(mask1, mask2, pattern1, pattern2, firstCount):
    ...
    dac.send_mod_BTF_int_list([int(mask1, 2), int(mask2, 2), 
                           int(pattern1, 2), int(pattern2, 2), 
                           after_trigger_count // 256, after_trigger_count % 256])
    dac.com.write(b'\x10T')

    dac.send_command('WAVEFORM')
    dac.com.write(b'\x10W') # Read waveform data
    try:
        dac.read_next_message() # This will raise b'\x10W' detection
    except Exception:
...
def test_trigger_and_read_multiple_captured_waveform(number):
    ...
    count = 0
    while(dac.com.inWaiting() > 0):
        try:
            count += dac.read_captured_waveform()
        except escapeSequenceDetected as e:
            if e.escape_char == 'W':

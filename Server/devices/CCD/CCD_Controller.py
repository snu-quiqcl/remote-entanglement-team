...
class DummyDacController(QThread):
    ...
    def __init__(self, logger=None, device="DummyCCD"):
        super().__init__()
        self.logger = logger
        self.queue = Queue()
            ...
            if work_type == "C":
                if command == "SETV":
                    ...
                    # Start acquisition
                        ...
                        if (data_size == 8 or data_size == 16):
                            msg = ["D", "CCD", "ERR", ["ACQ", 3]]
...
class AcquisitionHandler(QThread):
    ...
    def run(self):
        while True:
            ...
            buffer_image = np.copy(self.ccd.ccd_image)
                    
            if len(client_16bit_list):
                msg = ["D", "CCD", "DAT16", [buffer_image]]
                self.controller.informClients(msg, client_16bit_list)
                
            if len(client_8bit_list):
                max_ccd_image = np.amax(buffer_image)
                rescaled_ccd_image = (255*buffer_image/max_ccd_image).astype(np.uint8) # normalized to 8 bits
                msg = ["D", "CCD", "DAT8", [rescaled_ccd_image, max_ccd_image]] # the max value is attached for reference
